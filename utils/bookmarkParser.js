...
function parseHtmlBookmarks(htmlText) {
  ...
  function parseContainer(container, currentFolder = '') {
    if (!container) return;

    Array.from(container.children || []).forEach((child) => {
      if (!child || child.tagName !== 'DT') return;

      const folderNode = child.querySelector(':scope > H3');
      if (folderNode) {
        ...
        let nested = child.nextElementSibling;
        if (!nested || nested.tagName !== 'DL') {
          nested = child.querySelector(':scope > DL');
        }
        if (nested) {
          parseContainer(nested, folderPath);
        }
        return;
      }

      const linkNode = child.querySelector(':scope > A');
      if (!linkNode) return;
