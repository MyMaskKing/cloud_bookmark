...
**云端书签插件**是一款功能强大、设计精良的书签管理工具，致力于为您提供跨设备、多场景的云端书签同步体验。核心优势包括：

- **🌐 真正的云端同步**：基于WebDAV协议，支持坚果云、iCloud、Nextcloud等主流云存储服务，实现真正的多设备实时同步，让您的书签随时随地触手可及。

- **🎯 多场景管理**：独创的场景功能让您轻松隔离工作与生活，支持家庭、公司、学习等自定义场景，每个场景独立管理，互不干扰。

- **📁 智能文件夹排序**：文件夹排序后，书签会自动按新的文件夹顺序重新排列，保持数据一致性，让您的书签组织更加有序。

- **🔍 强大的搜索与筛选**：支持标题、URL、描述、备注、标签多维度搜索，可按文件夹、标签、收藏状态快速筛选，快速定位目标书签。

- **💾 完整的数据管理**：支持从浏览器书签栏、HTML、JSON文件导入，可导出为HTML或JSON格式，数据迁移无忧。

- **⚡ 实时同步与离线支持**：本地变更立即上传云端，支持离线模式，网络恢复后自动同步，确保数据不丢失。

- **🎨 灵活的界面定制**：支持网格/列表视图切换，可自定义显示内容（URL、描述、备注、标签等），适配PC和移动端，提供悬浮球快速入口。

- **🔒 隐私安全**：所有数据存储在您自己的WebDAV服务器上，插件不收集任何个人信息，完全掌控您的数据。

无论您是个人用户还是团队协作，云端书签插件都能为您提供专业、可靠的书签管理解决方案。
